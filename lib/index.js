...
module.exports = class FileWatcher extends EventEmitter {
  ...
  constructor(opts = {}) {
    ...
    this.folderPath = opts.folderPath || Config.RAW_DATA_FOLDER;
    this.folderPath = path.resolve(this.folderPath);

    this.extensionToLoad = opts.extensionToLoad || '.raw';
    this.currentState = FileWatcher.STATE_ALARM;
    ...
    this.monitor = new Monitor({
      ...
      operation: ({attempt}) => {
        ...
        if (timeDiff >= Config.connectMaxTimeout) {
          ...
          Logger.debug(`> refreshing folder: ${this.folderPath}...`);
          exec(`ls -la ${this.folderPath}`, (error, stdout, stderr) => {
            let errorMsg = '';
            let stderrMsg = '';
  ...
  onDirUpdate(name, stats) {
    ...
    if (stats.updated) {
      this.lastUpdateTime = stats.updated;
    }
  }

  start() {
    const {alertTimeDiff, filePollingInterval, fileWatchThreshold} = Config;
    ...
    this.dirWatcher = new DirWatcher({
      ...
      extensionToLoad: this.extensionToLoad,
      filePollingInterval,
      fileWatchThreshold
    });
    ...
    Logger.info(`Watcher started in ${currentState} state; waiting for updates on folder ${this.folderPath}...`);
  ...
  stateChange(newState, prevState, opts) {
    ...
    if (this.broadcastStateChange(newState, prevState)) {
      ...
      this.emit('stateChange', {state: this.currentState, prevState, ...opts});
    }
  }
